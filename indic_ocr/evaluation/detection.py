...
def json_to_pascalvoc(json_folder, gt_mode=False):
    ...
    for json_file in tqdm(json_files, desc=tqdm_str, unit=' images'):
        ...
        for bbox in bboxes:
            #w, h = bbox['width'], bbox['height'] # Just asserting if they exist because we have to work only with
            # rectangles
...
def run_pascal_voc_metrics(gt_folder_pascalvoc, pred_folder_pascalvoc):
    ...
    os.system('%s %s -gt %s -det %s' % ('python',PASVAL_VOC_EVAL_SCRIPT, gt, det))
    return
