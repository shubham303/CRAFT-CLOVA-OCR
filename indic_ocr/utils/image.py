...
def get_all_json_files(folder):
    files = glob(os.path.join(folder, '*'))
    data_files = []
    data_files += [file for file in files if file.lower().endswith(".json")]
    return data_files
...
# save img in output folder with same name as input image and append str to the name.
def saveImage(img, path, str):
    out_file = os.path.join(configuration.output_folder, os.path.splitext(os.path.basename(path))[0])
    cv2.imwrite("{}_{}.jpg".format(out_file,str), img)